...
<transition name="fade" appear>
    <div class="form" >
        <div class="top">
             <div style="font-size:20px" class="title">Photo de {{conjoint[0]?.nom_conjoint}} {{conjoint[0]?.prenom_conjoint}}</div>
...
export default {
    ...
    methods:{
        ...
        getConjoint(){
            ...
            .then((res)=>{
                this.$store.state.conjoint = res.data
        ...
        savePhoto(){
            const config = {
                ...
                this.loading = true;
                axios.post(this.url+'update_photo_conjoint/'+this.conjoint[0]?.id, data, config)
                .then((response)=>{
                this.loading = false;
                this.close();
                // this.getDistricts();
